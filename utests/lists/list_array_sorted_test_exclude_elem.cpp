...
namespace test_list_array_sorted
{
	...
	: public list_array_sorted_test_case
    {
	TEST_INSERTER;
	void configure()
	    {
	...
	test_result run()
		...
		score_t score = score_t(head_ok) + score_t(tail_ok) + head_cmp + tail_cmp;
		return score;
	    }
    };
	...
	test_result run()
		...
		bool tail_ok = test_plug.get_Tail(container) == 2;
		node_cmp_t head_cmp = test_node(head_ok, 0, {null,2,data[0]});
		node_cmp_t tail_cmp = test_node(tail_ok, 2, {0,null,data[2]});
		score_t score = score_t(head_ok) + score_t(tail_ok) + head_cmp + tail_cmp;
		return score;
	    }
    };
	...
	test_result run()
		...
		node_cmp_t tail_cmp = test_node(tail_ok, 1, {0,null,data[1]});
		score_t score = score_t(head_ok) + score_t(tail_ok) + head_cmp + tail_cmp;
		return score;
	    }
    };
	...
	test_result run()
		...
		node_cmp_t head_cmp = test_node(head_ok, 1, {null,0,data[1]});
	...
	test_result run()
		...
		bool tail_ok = test_plug.get_Tail(container) == 0;
		node_cmp_t head_cmp = test_node(head_ok, 2, {null,0,data[2]});
	...
	test_result run()
		...
		node_cmp_t tail_cmp = test_node(tail_ok, 1, {2,null,data[1]});
		score_t score = score_t(head_ok) + score_t(tail_ok) + head_cmp + tail_cmp;
		return score;
	    }
    };
