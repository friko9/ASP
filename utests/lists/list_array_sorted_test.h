...
{
    using list_t = list_array_sorted<T>;
    ...
    index_t find(list_array_sorted<T>& arg, T x) { return arg.find(x); }
    void move_elem(list_array_sorted<T>& arg,index_t src,index_t dst) { arg.move_elem(src,dst); }
    void exclude_elem(list_array_sorted<T>& arg,index_t n) { arg.exclude_elem(n); }
};
...
namespace test_list_array_sorted
{
    ...
    struct list_array_sorted_test_case
	: public test_case
    {
	using elem_t = int8_t;
	using TestPlug_t = TestPlug<list_array_sorted<elem_t>>;
    ...
    inline list_array_sorted_test_case::score_t operator +(list_array_sorted_test_case::score_t left,
							   list_array_sorted_test_case::score_t right)
