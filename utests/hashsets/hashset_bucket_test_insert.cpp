...
{
    struct hashset_bucket_test_insert_compare_with_stdhashset : public hashset_bucket_test_case
    {
	test_result run()
	    {
		...
		compare_test_range<It>( result,
					...
					auto& vmap = test_plug.get_vMap(container);
					auto cmp_end = this->testhashset.end();
					for(elem_t j = min; j < max; ++j)
					{
					    auto result = vmap.at(test_plug.find(container,j));
					    bool contains = std::find(result.begin(),result.end(),j) != result.end();
					...
					auto result = vmap.at(test_plug.find(container,max));
					bool contains = std::find(result.begin(),result.end(),max) != result.end();
	...
	: public hashset_bucket_test_insert_compare_with_stdhashset
    {
	TEST_INSERTER;
	void configure()
	    {
